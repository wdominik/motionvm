...
use motionvm_formats::m16::{Container, Generation, GfxInf, Segment, gfx, mz, psm, scr, text};
...
fn the_header_is_the_earlier_framing_and_says_so_by_adding_up() {
    ...
    assert_eq!(c.generation(), Generation::One);
...
fn this_builds_hot_area_test_has_no_hole_case() {
    ...
    for (other, expected) in [
        (motionvm_testutil::gamedata_enviro(), true),
        (motionvm_testutil::gamedata_jeffjet(), false),
        (motionvm_testutil::gamedata_hfa(), true),
    ] {
        let Some(other) = other else { continue };
        let exe = ["ENVIRO.EXE", "HPPLAY.EXE", "BMZ.EXE"]
            .into_iter()
            .find_map(|n| motionvm_formats::find_ci(&other, n))
