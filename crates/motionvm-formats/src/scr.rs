...
//! the high half, see [`crate::le`]) or a reference to a word in some module as
...
fn body_end(item: &[u8], start: usize, limit: usize) -> usize {
    ...
    while p + 4 <= limit {
        ...
        if cell >> 16 != crate::le::TAG_KERNEL >> 16 {
            continue;
        }
        let ordinal = cell & 0xffff;
        if crate::le::inline::takes_cell(ordinal) {
            p += 4;
        } else if crate::le::inline::takes_string(ordinal) {
