...
impl Palette {
    ...
    /// refused. That is deliberate and it is what the original does: the DAC
    /// gets however many triples the game hands it and the rest of the
    /// registers keep whatever they held. Refusing here would turn a palette
    /// the original displays into a startup failure.
    ///
    /// It is worth knowing this never happens on the shipped data — every
    /// palette in the containers is exactly 768 bytes, which
    /// `palettes_are_768_bytes_of_6bit_values` checks — so a short one is a
    /// sign of damage even though it is tolerated.
