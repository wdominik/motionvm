...
//! `NNN.RSC` containers, GFX8 sprites and the GFXCRUNCH LZW codec they and the
//! fonts are packed with, the 32-bit text table, the 32-bit script modules,
//! HMI songs, Ad Lib banks, driver archives and the LE binary the kernel
//! table is lifted from. What
//! both generations share — palettes, the font reference table, the decoded
//! font and text-table structures, the error type and the byte helpers — lives
//! at the crate root.
