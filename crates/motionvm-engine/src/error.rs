...
//! `motionvm_formats::Error` for a file that will not read as what it claims,
//! `motionvm_forth::Error` for a machine that cannot go on,
//! `motionvm_audio::Error` for the sound path. This crate flattened all of it
...
//!   [`crate::titles::open`], [`crate::Game`] and [`crate::Playable`] answer
//!   with. It is what `motionvm-app` and `motionvm-tools` see.
//! - **`motionvm_forth::Error`** is the *machine's*, and stays the machine's.
...
impl Error {
    ...
    pub(crate) fn data(path: &Path, source: motionvm_formats::Error) -> Self {
        Error::Data {
...
///
/// There is no `From<motionvm_formats::Error>` beside it on purpose: a reader
...
impl From<motionvm_forth::Error> for Error {
    fn from(e: motionvm_forth::Error) -> Self {
        Error::Machine(e)
    }
}
