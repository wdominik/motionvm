...
impl Family for Motion {
    ...
    /// Built from [`titles::Title::ALL`], so the roster a window shows cannot
    /// fall behind the roster the openers serve.
...
impl Playable for Front {
    ...
    fn key(&mut self, press: &motionvm_playable::KeyPress, down: bool) {
        self.game.key(press, down);
    }
    ...
    fn render(&mut self) -> motionvm_playable::Framebuffer {
        self.game.render()
    }

    fn palette(&self) -> &motionvm_playable::Palette {
        self.game.palette()
    }
    ...
    fn open_music(&mut self, rate: u32) -> Result<Option<Box<dyn AudioSource>>> {
        let (source, sink) = music::open_music(&self.dir, rate)?;
