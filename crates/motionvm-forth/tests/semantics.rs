...
/// position. The eight vectors are the ones recorded there, taken from putting
...
fn branch_targets_match_the_compiler() {
    ...
    assert_eq!(
        branch::target(inline::CHECK_IF, 3, 5),
        8,
        "IF to the ELSE arm"
    );
    assert_eq!(
        branch::target(inline::CHECK_ELSE, 7, 3),
        10,
        "ELSE past the ENDIF"
    );
    // : T 1 =IF 2 ENDIF ;
    assert_eq!(branch::target(inline::CHECK_EIF, 3, 3), 6, "=IF");
    // : T BEGIN 1 UNTIL ;
    assert_eq!(
        branch::target(inline::UNTIL, 3, 3),
        0,
        "UNTIL back to BEGIN"
    );
    // : T BEGIN 1 WHILE 2 REPEAT ;
    assert_eq!(branch::target(inline::LOOP_BREAK, 3, 5), 8, "WHILE out");
    assert_eq!(branch::target(inline::REPEAT, 7, 7), 0, "REPEAT back");
    // : T 0 10 DO 1 LOOP ;  /  : T 0 10 DO 1 2 +LOOP ;
    assert_eq!(branch::target(inline::LOOP_END, 8, 3), 5, "LOOP body");
    assert_eq!(branch::target(inline::ADD_LOOP, 10, 5), 5, "+LOOP body");
}

/// Forward and backward are a property of the word, not of the operand.
#[test]
fn only_four_branch_words_go_forward() {
    for f in [
        ...
        assert!(branch::is_forward(f), "{f} should go forward");
    }
    for b in [
        inline::UNTIL,
        inline::REPEAT,
        ...
        assert!(!branch::is_forward(b), "{b} should go backward");
        assert!(branch::is_branch(b), "{b} is still a branch");
    }
    assert!(
        !branch::is_branch(inline::PUT_LIT),
