...
//! [`motionvm_formats::Binding`] — and the primitives' names, which are the
...
/// type on both and differ only in what the machine behind it is; the
/// default is the 32-bit machine, which is the one the engine runs today.
pub trait Host<M = m32::Vm> {
