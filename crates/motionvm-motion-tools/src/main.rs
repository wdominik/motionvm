...
//! Jet, Hilfe für Amajambere or Victor Loomes, told apart by the engine binary
//! beside it) and goes to [`m16`]. The two command sets write the same kinds
...
use motionvm_motion_formats::{TextTable, font};
...
for Victor Loomes (all four MOTION 16-bit). --pal names the palette a 16-bit
...
///
/// Deliberately not the engine's `titles::detect`, and not to be replaced by
/// it: that probe answers *which game the player plays* and refuses a MOTION
/// game outside its roster, while this one answers *which machine wrote the
/// files* — the tools read more games than the player plays, Checker 2000
/// among them. The tools must never grow an engine dependency for it either;
/// both crates are the family's, so the boundary test would not object, and
/// the narrowing would be silent.
enum Generation {
