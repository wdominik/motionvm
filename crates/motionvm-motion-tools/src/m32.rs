...
fn engine_exe(dir: &Path) -> Result<std::path::PathBuf, Box<dyn std::error::Error>> {
    motionvm_motion_formats::find_ci(dir, "ENGINE.EXE")
...
pub(crate) fn extract(dir: &Path, out: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (n_text, over) = each_item(&bank, Kind::Text, |id, item| {
        let table = motionvm_motion_formats::m32::text::parse(item)?;
    ...
    let refs = motionvm_motion_formats::find_ci(dir, "000.FRT")
    ...
    for (name, n) in &usage {
        let have = motionvm_motion_forth::m32::IMPLEMENTED.contains(&name.as_str());
