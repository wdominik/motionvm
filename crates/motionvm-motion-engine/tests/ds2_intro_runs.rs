...
fn the_old_picture_fades_out_in_its_own_colors() {
    ...
    let load = |id: usize| {
        let item = bank
            .item(motionvm_motion_formats::m32::Kind::Palette, id)
            .expect("read")
            .expect("present");
        motionvm_render::Palette::from_6bit(item)
    };
...
fn a_fade_in_draws_only_its_own_screen() {
    ...
    let two = |e: &motionvm_motion_engine::Engine, x: i32| {
        e.screens()
            .iter()
            .find(|s| s.handle == 2)
            .expect("screen 2")
