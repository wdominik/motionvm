...
struct Bar {
    ...
    calcinv: motionvm_motion_forth::Address,
}
...
fn cell(base: u32, off: u32) -> motionvm_motion_forth::Address {
    motionvm_motion_forth::Address::new(base >> 16, (base & 0xffff).wrapping_add(off))
}
...
fn the_carried_items_slot_keeps_blinking() {
    ...
    fn slot<'a>(g: &'a Game<Vm>, b: &Bar) -> &'a motionvm_motion_engine::Descriptor {
        g.engine
            .descriptors()
            .iter()
...
fn a_savegame_says_what_the_bar_is_showing() {
    ...
    let Some((saves, slot)) = savegame_slot("ds2", &[702, 701, 703, 704, 705]) else {
