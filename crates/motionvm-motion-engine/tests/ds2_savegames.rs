...
fn game_with_saves(dir: &Path, name: &str) -> (Game<Vm>, PathBuf, PathBuf) {
    let base = saves_dir(&format!("ds2-{name}"));
    let mut game = Game::open(dir).expect("game opens");
    game.set_saves(&base)
        .expect("the save directory is accepted");
    // What the engine was pointed at, and where it decided the slots go — the
    // game's own name under it. Both, because one test points a second game at
    // the same place and the rest read the files.
...
fn exist_answers_minus_one_for_a_taken_slot() {
    ...
    let (mut game, _base, saves) = game_with_saves(&dir, "exist");
...
fn show_files_fills_the_slot_table() {
    ...
    let (mut game, _base, saves) = game_with_saves(&dir, "show_files");
...
fn started_in(dir: &Path, name: &str, location: i32) -> (Game<Vm>, PathBuf, PathBuf) {
    let (mut game, base, saves) = game_with_saves(dir, name);
    game.start().expect("4:START");
    while game.pump().expect("startup runs") {}
    game.set_var(2, "_NEXTLOC", location)
        .expect("ask for the location");
...
fn putas_writes_the_resident_modules_and_getas_puts_them_back() {
    ...
    let (mut game, _base, saves) = started_in(&dir, "putas", 23);
...
fn a_savegame_that_does_not_fit_is_refused_before_anything_changes() {
    ...
    let (mut game, _base, saves) = started_in(&dir, "refused", 23);
    game.set_var(11, "_?STIFT", 7).expect("a story flag");
    kernel(&mut game, "=>PUTAS", &[701]);
    game.set_var(11, "_?STIFT", 99).expect("change it again");
...
fn a_game_saves_and_loads_through_its_own_menu() {
    ...
    let (mut game, _base, saves) = started_in(&dir, "menu", 23);
    game.set_var(11, "_?STIFT", 7).expect("a story flag");
...
fn a_load_leaves_the_picture_running() {
    ...
    let (mut game, _base, _saves) = started_in(&dir, "running", 23);
    open_menu(&mut game, 4);
    click_slot(&mut game, 0);
    open_menu(&mut game, 3);
    click_slot(&mut game, 0);
...
fn a_savegame_loaded_into_a_fresh_game_hands_out_no_handle_twice() {
    ...
    let (mut far, base, _saves) = started_in(&dir, "handles", 23);
    ...
    fresh.set_saves(&base).expect("the same directory");
...
fn only_the_boot_modules_create_fonts_templates_and_screens() {
    ...
    let bank = motionvm_motion_formats::m32::rsc::Bank::open_dir(&dir).expect("resources");
    for word in ["+FONT", "DEFTDT", "NEWSCREEN", "DELAY", "CTRL", "STEPMULTI"] {
        let mut found = Vec::new();
        for (_, id) in bank.present(motionvm_motion_formats::m32::Kind::Script) {
            let item = bank
                .item(motionvm_motion_formats::m32::Kind::Script, id)
                .unwrap()
                .expect("present");
            let Ok(parsed) = motionvm_motion_formats::m32::ScrModule::parse(item) else {
                continue;
            };
            let uses = parsed.entries.iter().any(|e| {
                e.body.iter().any(|c| {
                    c >> 16 == motionvm_motion_formats::m32::le::TAG_KERNEL >> 16
