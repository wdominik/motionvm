...
fn text_measures_the_same_as_in_the_original() {
    ...
    let table = motionvm_motion_formats::m32::text::parse(item).expect("table parses");
    ...
    for (number, engine_width) in ORIGINAL_WIDTHS {
        ...
        let ours = motionvm_motion_engine::text::text_width(font, refs, text);
        assert_eq!(
...
fn the_gap_falls_between_glyphs_and_not_after_the_last() {
    ...
    assert_eq!(
        motionvm_motion_engine::text::text_width(font, refs, "MMMM"),
        4 * w + 3
    );
    assert_eq!(motionvm_motion_engine::text::text_width(font, refs, ""), 0);
}
