...
/// A big sprite from the game, plus a small one with transparency, as the
/// pictures the renderer takes — and the big one's own palette, which the
/// picture does not carry and the color chain still has to be checked against.
fn art() -> Option<(Picture, Picture, Palette)> {
    let bank = Bank::open_dir(gamedata_ds2()?).ok()?;
    let mut big: Option<Picture> = None;
    let mut palette: Option<Palette> = None;
    let mut masked: Option<Picture> = None;
    for (_, id) in bank.present(Kind::Gfx8) {
        let Ok(Some(item)) = bank.item(Kind::Gfx8, id) else {
            continue;
        };
        let Ok(s) = Sprite::parse(item) else { continue };
        let own = Palette::from_6bit(&s.palette);
        let s = Picture {
            width: s.width,
            height: s.height,
            pixels: s.pixels,
