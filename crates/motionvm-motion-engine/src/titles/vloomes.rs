...
//! ([`motionvm_motion_formats::m16::Framing::Earlier`]), which the reader tells from
//! the file. And it moves between locations through `NAO` and `AO` in module
//! 605 rather than `NEXTLOC` in module 601, which the later games use and this
//! one has no module for; the scheme below is where that is written down.
//!
...
/// on its own here — see [`motionvm_motion_formats::m16::gfxinf`].
...
///
/// Its own, and the reason this file has one at all. There is no module 601
/// here: `AO` is the location the game is in and `NAO` the one it has been
/// asked for, both in module 605, and `CTRL` ends every frame with
/// `NAO @ IF NAO @ INCLORT NAO 0! THEN` — this game's spelling of the
/// `NEXTLOC` poll the later builds' games run. There is no `STARTLOC` either:
/// `RUN` zeroes `AO` and enters the first location itself with `1 INCLORT`, so
/// `AO` is both what says a location has been entered and what says which,
