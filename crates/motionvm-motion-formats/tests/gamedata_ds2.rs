...
use motionvm_motion_formats::m32::{Kind, ScrModule, Sprite, rsc::Bank};
...
fn palettes_are_768_bytes_of_6bit_values() {
    ...
    for (_, id) in bank.present(Kind::Palette) {
        ...
        // 768: a whole VGA palette, 256 entries of three 6-bit channels.
        assert_eq!(item.len(), 768, "palette {id} length");
        assert!(
...
fn text_tables_parse_and_contain_german() {
    ...
    for (_, id) in bank.present(Kind::Text) {
        ...
        let t = motionvm_motion_formats::m32::text::parse(item)
...
fn fonts_decode_and_spell_the_alphabet() {
    ...
    for (_, id) in ids {
        ...
        let f = motionvm_motion_formats::m32::font::parse(item)
...
/// The inline ordinals the branch decoder runs on are the ones this kernel
/// hands out.
///
/// [`le::INLINE`] is the set measured on `ENGINE.EXE` V0.06.06/R109, and the
/// interpreter reads its own set out of whatever binary a game ships rather
/// than out of those constants — so the two agreeing is what makes reading
/// them from the binary cost nothing today, and a build that disagreed would
/// fail here instead of decoding an `IF` as a `+LOOP` somewhere in a scene.
#[test]
fn the_shipped_kernel_binds_the_inline_words_where_the_measurement_says() {
    let Some(dir) = gamedata_ds2() else {
        eprintln!("skipping: no Dunkle Schatten 2 gamedata directory");
        return;
    };
...
fn the_adlib_banks_parse_and_name_their_instruments() {
    ...
    let m = motionvm_motion_formats::m32::bnk::Bank::parse(&melodic).expect("melodic bank parses");
    assert_eq!(
        m.names.len(),
        128,
    ...
    let d = motionvm_motion_formats::m32::bnk::Bank::parse(&drum).expect("drum bank parses");
...
fn every_song_decodes_to_its_end_of_track() {
    ...
    for id in 0..=25 {
        let item = bank
            ...
            .expect("read block")
            .expect("song present");
        let song = motionvm_motion_formats::m32::hmi::Song::parse(item)
        ...
        for (t, track) in song.tracks.iter().enumerate() {
            ...
            assert_eq!(
                last.event,
                motionvm_motion_formats::m32::hmi::Event::EndOfTrack,
                "song {id} track {t} does not end on FF 2F"
            );
            for e in &track.events {
                match e.event {
                    motionvm_motion_formats::m32::hmi::Event::NoteOn { .. } => notes += 1,
                    motionvm_motion_formats::m32::hmi::Event::NoteOff { .. } => note_offs += 1,
                    _ => {}
                }
            }
            tracks += 1;
...
fn the_hmi_test_song_matches_its_midi_source() {
    ...
    let song = motionvm_motion_formats::m32::hmi::Song::parse(
    ...
    for track in &song.tracks {
        for e in &track.events {
            if let motionvm_motion_formats::m32::hmi::Event::NoteOn { note, velocity, .. } = e.event
            {
...
fn the_driver_archives_walk_to_their_last_byte() {
    ...
    for (name, count) in [("HMIMDRV.386", 8), ("HMIDET.386", 80), ("HMIDRV.386", 100)] {
        ...
        let arc = motionvm_motion_formats::m32::DriverArchive::parse(&bytes)
    ...
    let arc = motionvm_motion_formats::m32::DriverArchive::parse(&bytes).expect("walks");
    assert_eq!(
