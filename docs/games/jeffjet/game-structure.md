...
Eight verbs, the same eight the other 16-bit game has, as constants in module
...
with `GET`, `INCLLOC`, `GETANIM`, `=>GETAS`, which is the other 16-bit game's
...
so one game's slot in the other game's directory would be opened rather than
refused ([departures](../../departures.md)).

## Open questions

- What the first field of the animation catalogs 125–182 means. It is `0xFFFF`
  in several of them where the other game's catalogs hold small integers.
