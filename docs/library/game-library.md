...
with the [walking system](../engine/walking.md); the sequence animator
...
shows (see [Interaction machine](../engine/interaction.md)).
...
[Blocks](../formats/block.md)) and two bulk setters:
...
- `SCANITEM ( -- )` hands the complete UI state to the native
  ...
  [Interaction machine](../engine/interaction.md)). The result (the
...
[Transitions](../engine/transitions.md) for the fades in the same phases).
...
[Game structure](../engine/game-structure.md). Two details worth
pinning here:
