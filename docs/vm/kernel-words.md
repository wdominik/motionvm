...
| `?KEY ( -- k )` | A keyboard-hit probe; consumes nothing (`KEY` reads). **Answers an ASCII key code**, or zero when no key is waiting — not a flag. `ICTRL` opens with `?KEY DUP _AKTKEY !` (module 4, `0x022a0`) and every later test on that variable is against a code: 8 backspace (`0x051a0`), 13 Return, 27 Escape (`0x02c40`), 48…57 the teleport digits (`0x04e60`), 103 `g` (`0x04d40`), 105 `i` (`0x05020`). The `_AKTKEY @ 0 >` form is the "any key" test and reads correctly either way, which is what lets a boolean survive here unnoticed — with one in that variable, none of those keys does anything |
